...
class ImageAttacker:
    ...
    def apply_jpeg_compression(self, image_path, quality_factors=None):
        ...
        for qf in quality_factors:
            ...
            self.attack_images.append(
                output_filename)  # 从完整路径中提取文件 basename（包含扩展名），然后去除扩展名，得到纯文件名
    ...
    def apply_brightness(self, image_path, factors=None):
        ...
        for f in factors:
            output_path = os.path.join(self.output_dir or os.path.dirname(image_path),
                                       f"brightness_{f:.1f}_{base_name}.jpg")
    ...
    def apply_contrast(self, image_path, factors=None):
        ...
        for f in factors:
            output_path = os.path.join(self.output_dir or os.path.dirname(image_path),
                                       f"contrast_{f:.1f}_{base_name}.jpg")
        ...
        for i in range(num_attacks):
            ...
            for attack_func in attacks_to_combine:
                ...
                if attack_func == self._apply_blur:
                    ...
                    attacked_img = self._apply_paint(attacked_img, stroke_size=random.choice([3, 5]),
                                                     num_strokes=random.randint(30, 70))

            output_path = os.path.join(self.output_dir or os.path.dirname(image_path),
                                       f"hybrid_{i + 1}_{base_name}.jpg")
...
def main():
    ...
    parser.add_argument("--output", "-o", default=None,
                        help="Output directory (if not specified, save in same directory as source)")
    ...
    parser.add_argument("--pkl", "-p", default=None,
                        help="Output PKL file path (if not specified, derive from JSON path)")
