# ============ app_paddle.py - PaddlePaddle Web 应用 ============
"""
PaddlePaddle Web 检索应用
使用方法: python app_paddle.py
"""
...
from model.ours_distill_CIBHash_paddle import CIBHash as StudentModel
from model.CIBHash_paddle import CIBHash as TeacherModel
from utils.data_paddle import LabeledData
from utils.evaluation_paddle import calculate_hamming
...
def load_system():
    ...
    hparams.cuda = paddle.device.is_compiled_with_cuda()
    hparams.device = 0
    ...
    try:
        ...
        teacher_path = f'./checkpoints/{hparams.data_name}_{hparams.t_model_name}_bit{hparams.encode_length}.pdparams'
    ...
    try:
        ...
        student_path = f'./checkpoints/ours_distill_{hparams.data_name}_{hparams.s_model_name}_{hparams.t_model_name}__1_bit_{hparams.encode_length}.pdparams'
        student_model = StudentModel(hparams)
        student_model.define_parameters()
        state_dict = paddle.load(student_path)
        student_model.set_state_dict(state_dict)
        student_model.eval()
    ...
    with paddle.no_grad():
        for data, _, _ in db_loader:
            data_gpu = data.astype('float32')
            if GLOBAL_DATA["teacher_model"]:
                db_hashes_teacher.append(GLOBAL_DATA["teacher_model"].encode_discrete(data_gpu).numpy())
            if GLOBAL_DATA["student_model"]:
                db_hashes_student.append(GLOBAL_DATA["student_model"].encode_discrete(data_gpu).numpy())
...
def upload_and_search():
    ...
    image_tensor = GLOBAL_DATA["test_transform"](query_image).unsqueeze(0).astype('float32')
    ...
    with paddle.no_grad():
        query_hash = model.encode_discrete(image_tensor).numpy()
...
def compare_images():
    ...
    img1_tensor = GLOBAL_DATA["test_transform"](img1).unsqueeze(0).astype('float32')
    img2_tensor = GLOBAL_DATA["test_transform"](img2).unsqueeze(0).astype('float32')
    ...
    with paddle.no_grad():
        hash1 = model.encode_discrete(img1_tensor).numpy()
        hash2 = model.encode_discrete(img2_tensor).numpy()
